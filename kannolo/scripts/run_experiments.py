...
def compile_rust_code(configs, experiment_dir):
    ...
    compile_command = configs.get("compile-command", "RUSTFLAGS='-C target-cpu=native' cargo build --release --features cli")
...
def get_index_filename(base_filename, configs):
    ...
    # Encode the determinants of the on-disk index format so that indexes built with different
    # encodings (e.g. plain/f16 vs dotpacking8-scalaru8) never collide on disk. These are
    # appended only when present, so configs that omit them keep their previous filenames.
    if "encoder" in configs:
        name.append(f"encoder_{configs['encoder']}")
    if "value-type" in configs:
        name.append(f"value-type_{configs['value-type']}")
    if configs.get("dataset-type") == "sparse" and "component-type" in configs:
        name.append(f"component-type_{configs['component-type']}")
...
def get_permutation_filename(configs):
    """Return the path of the EGB permutation .npy file for this index."""
    index_file = os.path.join(
        configs["folder"]["index"],
        get_index_filename(configs["filename"]["index"], configs),
    )
    return f"{index_file}_permutation.npy"


def build_permutation(configs, experiment_dir):
    """Run hnsw_egb_permutation to generate the EGB permutation file."""
    permutation_command = configs.get("permutation-command", None)
    if not permutation_command:
        raise ValueError("permutation-command must be specified to build the permutation.")

    index_file = os.path.join(
        configs["folder"]["index"],
        get_index_filename(configs["filename"]["index"], configs),
    )
    output_file = get_permutation_filename(configs)

    metric = configs["indexing_parameters"]["metric"]
    if metric in ("l2", "ip"):
        metric = "euclidean" if metric == "l2" else "dotproduct"
    ...
    if "encoder" in configs:
        command_and_params.append(f"--encoder {configs['encoder']}")
    if configs.get("encoder") == "pq" and "pq_parameters" in configs and "pq-subspaces" in configs["pq_parameters"]:
    ...
    print()
    print(colored("Building EGB permutation", "green"))
    print(colored("Permutation command:", "blue"), command)

    permutation_output_file = os.path.join(experiment_dir, "permutation.output")
    with open(permutation_output_file, "w") as perm_out:
        perm_process = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        for line in iter(perm_process.stdout.readline, b''):
            decoded_line = line.decode()
            print(decoded_line, end='')
            perm_out.write(decoded_line)
        perm_process.stdout.close()
        perm_process.wait()

    if perm_process.returncode != 0:
        print(colored("ERROR: Permutation build failed!", "red"))
        sys.exit(1)

    print(colored(f"Permutation written to {output_file}", "yellow"))
...
def query_execution(configs, query_config, experiment_dir, subsection_name, subsection_index=None, total_subsections=None):
    ...
    if not is_reranking_binary:
        ...
        if configs.get("dataset-type") == "sparse" and "component-type" in configs:
            command_and_params.append(f"--component-type {configs['component-type']}")
        if "encoder" in configs:
            # In-memory convert: keep the source index/permutation names derived from the plain
            # `encoder` (e.g. plain-f16 or dotpacking8-scalaru8), but tell the query binary to
            # convert that loaded index to `convert_target_encoder` before searching.
            convert_target = configs.get("settings", {}).get("convert_target_encoder")
            if convert_target:
                command_and_params.append(f"--encoder {convert_target}")
                command_and_params.append("--convert-in-memory")
            else:
                command_and_params.append(f"--encoder {configs['encoder']}")
        if "graph-type" in configs:
            command_and_params.append(f"--graph-type {configs['graph-type']}")
        ...
        # For PQ encoder: enable two-stage search if a rerank dataset is configured
        if configs.get("encoder") == "pq" and configs.get("filename", {}).get("rerank-dataset"):
            rerank_dataset_path = os.path.join(configs["folder"]["data"], configs["filename"]["rerank-dataset"])
            command_and_params.append(f"--rerank-dataset {rerank_dataset_path}")
            k_candidates = query_config.get("k-candidates") or query_config.get("k_candidates")
            if k_candidates is not None:
                command_and_params.append(f"--k-candidates {k_candidates}")

        # Add graph compression parameters from per-query config
        if "compression" in query_config:
            command_and_params.append(f"--compression {query_config['compression']}")
        if query_config.get("permute", False):
            command_and_params.append(f"--permutation-file {get_permutation_filename(configs)}")
        if "zeta-k" in query_config:
            command_and_params.append(f"--zeta-k {query_config['zeta-k']}")
    ...
    with open(log_output_file, "w") as log:
        ...
        for line in iter(query_process.stdout.readline, b''):
            ...
            match = re.search(r"Total: (\d+) bytes", decoded_line)
            if match:
                memory_usage = int(match.group(1))
            match = re.search(r"\[######\] Size: ([\d.]+) bits/edge", decoded_line)
            if match:
    ...
    return query_time, bits_per_edge, accuracy, metric_value, memory_usage
...
def get_machine_info(configs, experiment_folder):
    ...
    if shutil.which("cpufreq-info") is None:
        machine_info.write("cpufreq-info not installed on this machine; skipping governor check.\n")
    else:
        ...
        cpus_with_performance_governor = None
        ...
        if cpus_with_performance_governor is not None and num_cpus != cpus_with_performance_governor:
            print()
...
def run_experiment(config_data):
   ...
   #print(config_data)
    ...
    if config_data['settings'].get('permutation', False):
        build_permutation(config_data, experiment_folder)
    ...
    with open(os.path.join(experiment_folder, "report.tsv"), 'w') as report_file:
        metric_header = f"\t{metric}" if has_metric else ""
        ...
        if 'query' in config_data:
            ...
            for subsection_index, (subsection, query_config) in enumerate(config_data['query'].items(), start=1):
                query_time, bits_per_edge, recall, metric_value, memory_usage = query_execution(config_data, query_config, experiment_folder, subsection, subsection_index, total_subsections)
                if has_metric:
                    report_file.write(f"{subsection}\t{query_time}\t{bits_per_edge}\t{recall}\t{metric_value or ''}\t{memory_usage}\t{building_time}\n")
                else:
