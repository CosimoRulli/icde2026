        ...
        for build_folder in sorted(build_folders):
            ...
            for _, row in report_df.iterrows():
                ...
                final_row = {
                    ...
                    # Top-level config keys that determine the encoding (not part of
                    # indexing_parameters), so f16 baseline and dotpacking8 rows are distinguishable.
                    "encoder": config.get("encoder", ""),
                    "value-type": config.get("value-type", ""),
                }
