...
fn main() {
    ...
    match (
        ...
        (DatasetType::Sparse, EncoderType::Dotpacking8Scalaru8, _, GraphType::Standard) => {
            build_sparse_dotpacking8::<Graph>(&args, distance, &config);
        }
        (DatasetType::Sparse, EncoderType::Dotpacking8Scalaru8, _, GraphType::FixedDegree) => {
            build_sparse_dotpacking8::<GraphFixedDegree>(&args, distance, &config);
        }
        // Sparse blocked-sparse (f16 blocked dot-product; value-type ignored)
        (DatasetType::Sparse, EncoderType::BlockedSparse, _, GraphType::Standard) => {
            build_sparse_blocked_sparse::<Graph>(&args, distance, &config);
        }
        (DatasetType::Sparse, EncoderType::BlockedSparse, _, GraphType::FixedDegree) => {
            build_sparse_blocked_sparse::<GraphFixedDegree>(&args, distance, &config);
        }
        // Sparse dotpacking8 sparse-scalar-u8 (value-type ignored)
        (DatasetType::Sparse, EncoderType::DotpackingSparseScalaru8, _, GraphType::Standard) => {
            build_sparse_dotpacking_sparse::<Graph>(&args, distance, &config);
        }
        (DatasetType::Sparse, EncoderType::DotpackingSparseScalaru8, _, GraphType::FixedDegree) => {
            build_sparse_dotpacking_sparse::<GraphFixedDegree>(&args, distance, &config);
        }
        // Unreachable: caught by earlier validation
        (DatasetType::Dense, EncoderType::Dotvbyte, _, _)
        | (DatasetType::Dense, EncoderType::Dotpacking8Scalaru8, _, _)
        | (DatasetType::Dense, EncoderType::BlockedSparse, _, _)
        | (DatasetType::Dense, EncoderType::DotpackingSparseScalaru8, _, _)
...
// --- DotPacking8 scalar-u8 (encoder = dotpacking8-scalaru8, DotProduct only) ---

fn build_sparse_dotpacking8<G>(args: &Args, distance: Distance, config: &HNSWBuildConfiguration)
where
    G: GraphBound,
{
    match distance {
        ...
        Distance::DotProduct => build_sparse_dotpacking8_dp::<G>(args, config),
    }
}

fn build_sparse_dotpacking8_dp<G>(args: &Args, config: &HNSWBuildConfiguration)
where
    G: GraphBound,
{
    let dataset: PlainSparseDataset<u16, f32, DotProduct> =
    ...
    let start_time = Instant::now();
    let plain_index: HNSW<_, G> = HNSW::build_index(dataset, config);
    // ConvertFrom for PackedSparseDataset<DotPacking8ScalarU8Encoder> trains the quantizer
    // internally (vectorium impl_from_packed_sparse_dataset_f32!), so no manual train step here.
    let index: HNSW<PackedSparseDataset<DotPacking8ScalarU8Encoder>, G> =
        plain_index.convert_dataset_into();
    let duration = start_time.elapsed();
    println!(
        "Time to build: {} s (before serializing)",
...
fn build_sparse_blocked_sparse<G>(args: &Args, distance: Distance, config: &HNSWBuildConfiguration)
where
    G: GraphBound,
{
    match distance {
        ...
        Distance::DotProduct => build_sparse_blocked_sparse_dp::<G>(args, config),
    }
}

fn build_sparse_blocked_sparse_dp<G>(args: &Args, config: &HNSWBuildConfiguration)
where
    G: GraphBound,
{
    let dataset: PlainSparseDataset<u16, f32, DotProduct> =
    ...
    let start_time = Instant::now();
    let plain_index: HNSW<_, G> = HNSW::build_index(dataset, config);
    // ConvertFrom for PackedSparseDataset<BlockedSparseEncoder> narrows the values to f16
    // internally (vectorium packed_dataset.rs), so no manual conversion step here.
    let index: HNSW<PackedSparseDataset<BlockedSparseEncoder>, G> =
        plain_index.convert_dataset_into();
    let duration = start_time.elapsed();
    println!(
        "Time to build: {} s (before serializing)",
...
fn build_sparse_dotpacking_sparse<G>(
    args: &Args,
    distance: Distance,
    config: &HNSWBuildConfiguration,
) where
    G: GraphBound,
{
    match distance {
        ...
        Distance::DotProduct => build_sparse_dotpacking_sparse_dp::<G>(args, config),
    }
}

fn build_sparse_dotpacking_sparse_dp<G>(args: &Args, config: &HNSWBuildConfiguration)
where
    G: GraphBound,
{
    let dataset: PlainSparseDataset<u16, f32, DotProduct> =
    ...
    let start_time = Instant::now();
    let plain_index: HNSW<_, G> = HNSW::build_index(dataset, config);
    // ConvertFrom (impl_from_packed_sparse_dataset_f32!) trains the quantizer internally,
    // so no manual train step here.
    let index: HNSW<PackedSparseDataset<DotPackingSparseScalarU8Encoder>, G> =
        plain_index.convert_dataset_into();
    let duration = start_time.elapsed();
    println!(
        "Time to build: {} s (before serializing)",
