...
use crate::graph::graph::{Graph, GraphTrait, GrowableGraph};
use crate::hnsw_utils::PackedArray;
...
pub struct HNSW<D, G> {
    ...
    /// Inverse permutation applied to the ground level after EGB reordering.
    /// Maps each ground-level local ID back to its original external ID.
    /// `None` when the index was not built with a permuted dataset.
    #[serde(default)]
    ground_inverse_permutation: Option<PackedArray>,
...
{
    /// Returns the ground level graph (densest layer).
    pub fn get_level0(&self) -> &G {
    ...
    #[must_use]
    #[inline]
    pub fn get_ground_external_id(&self, local_id: usize) -> usize {
        self.ground_inverse_permutation
            .as_ref()
    ...
    G: GraphTrait + From<GrowableGraph>,
{
    ...
    /// Replace the dataset with one produced by `f`, keeping the graph structure intact.
    ///
    /// Like [`convert_dataset_into`](Self::convert_dataset_into), but driven by an arbitrary
    /// closure instead of a `ConvertFrom` impl. Useful to bridge encodings that have no direct
    /// `ConvertFrom` (e.g. decoding a packed u8 dataset to plain f32 before re-encoding). The
    /// caller must ensure the produced dataset has the same number of vectors and the same
    /// logical vector order as the original.
    pub fn map_dataset<T, F>(self, f: F) -> HNSW<T, G>
    where
        T: Dataset,
        F: FnOnce(D) -> T,
    {
...
impl<D, Nsrc, Ndst> From<&HNSW<D, Graph<Nsrc>>> for HNSW<D, Graph<Ndst>>
where
    D: Dataset + Clone,
    Nsrc: Neighbors + Sync,
    Ndst: Neighbors + From<NeighborData>,
{
    fn from(src: &HNSW<D, Graph<Nsrc>>) -> Self {
        let mut new_levels = Vec::with_capacity(src.levels.len());

        for level in src.levels.iter() {
            new_levels.push(Graph::<Ndst>::from(level));
        }

        HNSW {
            levels: new_levels.into_boxed_slice(),
            level1_to_level0_mapping: src.level1_to_level0_mapping.clone(),
            ground_inverse_permutation: src.ground_inverse_permutation.clone(),
            dataset: src.dataset.clone(),
            num_neighbors_per_vec: src.num_neighbors_per_vec,
...
where
    ...
    Nsrc: Neighbors + Sync,
{
    pub fn remap_compress_and_replace_dataset_with_permutation<Ndst, Ddst, F>(
        &self,
        permutation: &[usize],
        make_dataset: F,
        ...
        Ndst: Neighbors + From<NeighborData>,
    {
        ...
        let mut previous_upper_old_globals_in_new_order = Vec::new();
        let mut ground_inv: Vec<usize> = Vec::new();

        for (i, level) in self.levels.iter().enumerate() {
            if i == last {
                let (ground, inv): (Graph<Ndst>, Vec<usize>) =
                    level.remap_ground_and_compress_with_permutation::<Ndst>(permutation);
                ground_inv = inv;
                new_levels.push(ground);
            } else {
                let old_locals_by_new_local = upper_level_order_preserving_hnsw_prefixes(
                    level,
                    permutation,
                    &previous_upper_old_globals_in_new_order,
                );
                let (remapped_level, local_mapping): (Graph<Ndst>, Vec<usize>) =
                    level.remap_level_and_compress_with_old_local_order::<Ndst>(
                        &old_locals_by_new_local,
                        permutation,
                    );
                previous_upper_old_globals_in_new_order = old_locals_by_new_local
                    .iter()
        ...
        HNSW {
            levels: new_levels.into_boxed_slice(),
            level1_to_level0_mapping,
            ground_inverse_permutation: Some(PackedArray::from(ground_inv)),
...
fn upper_level_order_preserving_hnsw_prefixes<G>(
    level: &G,
    permutation: &[usize],
    ...
    G: GraphTrait + From<GrowableGraph>,
{
        ...
        F: Fn(usize) -> bool,
    {
        ...
        let mapped_predicate = |local_id: usize| predicate(self.get_ground_external_id(local_id));
        ...
        topk.drain(..)
            .map(|candidate| vectorium::dataset::ScoredVector {
                ...
                vector: self.get_ground_external_id(candidate.vector) as VectorId,
            })
            .collect()
    }
}
    ...
    G: GraphTrait + From<GrowableGraph>,
{
    ...
    fn print_space_usage_bytes(&self) {
        ...
        let inv_perm_size = self.ground_inverse_permutation
            .as_ref()
            .map_or(0, PackedArray::byte_size);
        let index_size: usize = self.levels.iter().map(|g| g.get_space_usage_bytes()).sum::<usize>()
            + inv_perm_size;
        ...
        let ground_level = self.levels.last().unwrap();
        let ground_size = ground_level.get_space_usage_bytes();
        let ground_nodes = ground_level.n_nodes();
        let ground_edges = ground_level.n_edges();
        let ground_neighbor_storage = ground_level.neighbors_size_bytes();
        let levels_size = index_size - ground_size;
        println!(
            "[######] Ground level size graph: {} bytes, neighbor storage: {} bytes, Levels size {} bytes ({} nodes, {} edges)",
            ground_size, ground_neighbor_storage, levels_size, ground_nodes, ground_edges
        );

        let total_nodes: usize = self.levels.iter().map(|g| g.n_nodes()).sum();
        let total_edges: usize = self.levels.iter().map(|g| g.n_edges()).sum();
        let total_neighbor_bytes: usize = self.levels.iter().map(|g| g.neighbors_size_bytes()).sum();
        ...
        println!(
            "[######] Avg deg = {:.2}, {:.2} bits/id ({} nodes, {} edges)",
            total_edges as f64 / total_nodes as f64,
            (total_neighbor_bytes * 8) as f64 / total_nodes as f64,
    ...
    fn build_index(dataset: D, build_params: &Self::BuildParams) -> Self {
        ...
        Self {
            ...
            ground_inverse_permutation: None,
            dataset,
